{if count($aStreamEvents)}
	{foreach from=$aStreamEvents item=oStreamEvent}		
		...
		<li class="stream-item stream-item-type-{$oStreamEvent->getEventType()}">
			...
			<p class="info"><a href="{$oUser->getUserWebPath()}"><strong>{$oUser->getLogin()}</strong></a>&nbsp;&nbsp;<span class="date" title="{date_format date=$oStreamEvent->getDateAdded()}">{date_format date=$oStreamEvent->getDateAdded() hours_back="12" minutes_back="60" now="60" day="day H:i" format="j F Y, H:i"}</span></p>
