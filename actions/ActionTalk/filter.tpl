<div class="talk-search" id="block_talk_search">
	...
	<div class="talk-search-content" id="block_talk_search_content" {if $_aRequest.submit_talk_filter}style="display:block;" {/if}>
		<form action="{router page='talk'}" method="GET" name="talk_filter_form">
			...
			<button type="submit" name="submit_talk_filter" value="" class="button button-primary">{$aLang.talk_filter_submit}</button>
			<button type="submit" name="" value="" class="button" onclick="return ls.talk.clearFilter();" >{$aLang.talk_filter_submit_clear}</button>
		</form>
	</div>
</div>